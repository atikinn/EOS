...
static
char 
uartGetchar(UART_MemMapPtr uartChannel) {
...
static
void 
uartPutchar(UART_MemMapPtr uartChannel, char ch) {
...
static
int 
uartGetcharPresent(UART_MemMapPtr uartChannel) {
...
static
void 
uartPuts(UART_MemMapPtr uartChannel, char *p) {
	while(*p) {
...
uart_read (struct vnode *vn, char *byte, int offset) {
    (void)offset;
    *byte = uartGetchar(UART2_BASE_PTR);
    return 0;
}
...
uart_write (struct vnode *vn, char byte) {
	(void)vn;
    uartPutchar(UART2_BASE_PTR, byte);
