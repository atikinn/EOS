...
void
touch_init(struct vnode *vn) {
	TSI_Init();
	TSI_Calibrate();
}

int
touch_read (struct vnode *vn, char *byte, int offset) {
    (void)offset;
    *byte = electrode_in(vn->dev->driver->id - 7);
    return 0;
}

int
touch_write (struct vnode *vn, char byte) {
	kprintf(STDOUT, "you can't write to this device\r\n");
	return 1;
}

const struct vfs_ops touch_ops = {
    .open  = dev_open,
    .close = dev_close,
    .write = touch_write,
    .read  = touch_read,
